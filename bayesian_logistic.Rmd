...
# Fit Bayesian Logistic Regression Model with Horseshoe Prior
hs_prior <- hs()
fit_hs <- stan_glm(outcome ~ ., data = train_data, family = binomial(link = "logit"), prior = hs_prior, chains = 2, iter = 1000, seed = 213, cores = 8)

# Summarize the model
print(summary(fit_hs))

# Variable Selection Using projpred with k-fold cross-validation
k_folds <- 10
cv_varsel <- cv_varsel(fit_hs, method = "forward", K = k_folds, refit_prj = FALSE, ncores = 843s)
print(cv_varsel)
