...
# Use a smaller sample for initial testing
sample_size <- min(100, nrow(dt))  # Use 100 samples or the full data if less than 100
set.seed(213)
...
fit_normal <- stan_glm(outcome ~ ., data = train_data, family = binomial(link = "logit"), prior = normal_prior, chains = 2, iter = 1000, seed = 213, cores = 8, thin = 2)
...
# Use a very small sample for initial testing
sample_size <- min(20, nrow(dt))  # Use 20 samples or the full data if less than 20
set.seed(213)
sample_idx <- sample(1:nrow(dt), size = sample_size)
sample_data <- dt[sample_idx]

# Identify and remove constant columns
constant_cols <- sapply(sample_data, function(x) length(unique(x)) == 1)
sample_data <- sample_data[, !constant_cols, with = FALSE]
...
# Fit Bayesian Logistic Regression Model with Normal Prior
normal_prior <- normal(location = 0, scale = 1)
fit_normal <- stan_glm(outcome ~ ., data = train_data, family = binomial(link = "logit"), prior = normal_prior, chains = 1, iter = 500, seed = 213, cores = 8, thin = 1)
...
# Variable Selection Using projpred with 2-fold CV and no refit_prj
k_folds <- 2
cv_varsel <- cv_varsel(fit_normal, method = "forward", K = k_folds, refit_prj = FALSE, ncores = 2)
print(cv_varsel)
...
# Predict on the Train Data
train_preds <- posterior_predict(fit_normal, newdata = train_data, draws = 100)
train_preds_prob <- apply(train_preds, 2, mean)
...
# Predict on the Test Data
test_preds <- posterior_predict(fit_normal, newdata = test_data, draws = 100)
test_preds_prob <- apply(test_preds, 2, mean)
