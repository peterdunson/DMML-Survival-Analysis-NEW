...
# Fit Bayesian Logistic Regression Model with Normal Prior
normal_prior <- normal(location = 0, scale = 1)
fit_normal <- stan_glm(outcome ~ ., data = train_data, family = binomial(link = "logit"), prior = normal_prior, chains = 1, iter = 400, seed = 213, cores = 8, thin = 1)

# Summarize the model
print(summary(fit_normal))

# Perform Variable Selection with 2-fold CV and no refit_prj
cv_varsel <- cv_varsel(fit_normal, method = "forward", K = 2, refit_prj = FALSE, ncores = 8)
print(cv_varsel)
