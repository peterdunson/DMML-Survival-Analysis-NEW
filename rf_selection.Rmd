...
# Extract the cross-validated AUC
auc <- max(rf_cv_model$results$ROC)
cat("Cross-validated AUC:", auc, "\n")

# Extract the cross-validated log-loss
log_loss <- function(y_true, y_prob) {
  epsilon <- 1e-15
  y_prob <- pmax(epsilon, pmin(1 - epsilon, y_prob))
  -mean(y_true * log(y_prob) + (1 - y_true) * log(1 - y_prob))
...
probs <- predictions[, make.names(levels(y_rf)[2])]  # Assuming the positive class is the second level

# Convert factor levels to 0 and 1 for log-loss calculation
y_true <- ifelse(predictions$obs == levels(y_rf)[2], 1, 0)
log_loss_value <- log_loss(y_true, probs)
cat("Cross-validated log-loss:", log_loss_value, "\n")
...
# Generate learning curves
train_sizes <- seq(0.1, 0.9, by = 0.1)  # Use up to 0.9 to avoid issues with full dataset
train_scores <- numeric(length(train_sizes))
test_scores <- numeric(length(train_sizes))
...
for (i in seq_along(train_sizes)) {
  ...
  x_train_part <- x_rf[train_idx, ]
  y_train_part <- y_rf[train_idx]
  x_test_part <- x_rf[-train_idx, ]
  y_test_part <- y_rf[-train_idx]
  
  # Ensure x_test_part is not empty
  if (nrow(x_test_part) == 0) {
    next
  }
  ...
  train_pred <- predict(rf_cv_model_part, newdata = x_train_part, type = "prob")[, 2]  # Probability of positive class
  test_pred <- predict(rf_cv_model_part, newdata = x_test_part, type = "prob")[, 2]  # Probability of positive class
  
  # Calculate AUC for training and test sets
  train_scores[i] <- roc(as.numeric(y_train_part) - 1, train_pred)$auc
  test_scores[i] <- roc(as.numeric(y_test_part) - 1, test_pred)$auc
}
