...
for (i in seq_along(train_sizes)) {
  ...
  if (length(train_idx) > 0 && nrow(x_train_part) > 0 && nrow(x_rf[-train_idx, ]) > 0) {
    ...
    test_pred <- predict(rf_cv_model_part, newdata = x_rf[-train_idx, ], type = "prob")[, 2]  # Probabilities for positive class

    # Calculate AUC for training and test sets
    train_scores[i] <- roc(as.numeric(y_train_part) - 1, train_pred)$auc
    test_scores[i] <- roc(as.numeric(y_rf[-train_idx]) - 1, test_pred)$auc
  } else {
    train_scores[i] <- NA
    test_scores[i] <- NA
  }
...
full_train_pred <- predict(rf_cv_model, newdata = x_rf, type = "prob")[, 2]
full_train_pred_class <- ifelse(full_train_pred > 0.5, 1, 0)
conf_matrix <- table(Predicted = full_train_pred_class, Actual = as.numeric(y_rf) - 1)
print(conf_matrix)
